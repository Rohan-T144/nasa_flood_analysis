...
parser.add_argument('-c', '--config', default='flood.yml', type=str, metavar='FILE',
...
def main():
    ...
    if args.distributed and args.device == 'cuda':
        ...
        _logger.warning("Distributed training requested but not on a CUDA device. Running in single process mode.")
        args.distributed = False

    if not args.distributed:
        _logger.info(f'Training with a single process on {args.device} device.')

    # The prefetcher in timm is CUDA-specific, so we disable it for other devices.
    if args.device != 'cuda':
    ...
    if mixup_active:
        ...
        # Create a collate function for mixup
    ...
    loader_train = create_loader(
        ...
        no_aug=True, # Important: disable timm's augmentation
