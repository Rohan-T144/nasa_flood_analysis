...
#rom timm.models.layers import to_2tuple, trunc_normal_, DropPath
...
__all__ = ['spikformer_segmentation']
...
class SimpleDecoder(nn.Module):
    def __init__(self, in_channels, num_classes):
        super().__init__()
        # Example: Upsample from 1/16 scale to full resolution
        ...
        self.upconv3 = nn.ConvTranspose2d(in_channels // 4, in_channels // 8, kernel_size=2, stride=2)
        self.upconv4 = nn.ConvTranspose2d(in_channels // 8, num_classes, kernel_size=2, stride=2)

    def forward(self, x):
        # Input x shape: (T*B, C, H, W)
        x = self.upconv1(x)
        x = self.upconv2(x)
        x = self.upconv3(x)
        x = self.upconv4(x)
...
class Spikformer(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], **kwargs
                 ):
        super().__init__()
        ...
        self.decoder = SimpleDecoder(in_channels=embed_dims, num_classes=num_classes)
    ...
    def forward(self, x):
        ...
        # features shape: (T, B, C, H', W')

        # Reshape for the decoder
        T, B, C, H, W = features.shape
        features_flat = features.flatten(0, 1) # Shape: (T*B, C, H', W')

        # Pass through the decoder
        masks = self.decoder(features_flat)
        # masks shape: (T*B, num_classes, H_orig, W_orig)

        # Reshape back and average over time dimension for final output
        _, _, H_out, W_out = masks.shape
        masks = masks.view(T, B, -1, H_out, W_out)
        final_mask = masks.mean(0) # Shape: (B, num_classes, H_orig, W_orig)

        return final_mask


@register_model
def spikformer_segmentation(pretrained=False, **kwargs):
    model = Spikformer(
        img_size_h=224, img_size_w=224,
        patch_size=16, embed_dims=512, num_heads=8, mlp_ratios=4,
        in_channels=3, num_classes=1, qkv_bias=False,
